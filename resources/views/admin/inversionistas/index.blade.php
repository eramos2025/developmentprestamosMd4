...
<div class="row">
    <div class="col-md-10">
        ...
        <div class="card-header">
            <h3 class="card-title">Inversionistas registrados</h3>
            <div class="card-tools">
                <a href="{{ url('/admin/inversionistas/create') }}" class="btn btn-primary"> Crear nuevo</a> 
            </div>
            <!-- /.card-tools -->
        </div>
 <!-- /.card-header -->
<div class="card-body">
    ...
    @foreach ($inversionistas as $inversionista)
        <tr>
            <td>{{ $contador++}} </td>
            <td>{{ $inversionista->nro_documento }} </td>
            ...
            <td>
                <div class="btn-group" role="group" aria-label="Basic example">
                    {{-- Ver --}}
                    <a href="{{ url('/admin/inversionistas/'.$inversionista->id) }}" class="btn btn-info btn-sm">
                        <i class="fa fa-eye"></i>
                    </a>

                    {{-- Editar --}}
                    <a href="{{ url('/admin/inversionistas/'.$inversionista->id . '/edit') }}" class="btn btn-success btn-sm">
                        <i class="fa fa-pencil-alt"></i>
                    </a>

                    {{-- Eliminar --}}
                    <form action="{{ url('/admin/inversionistas/'.$inversionista->id) }}" method="POST" id="miFormulario{{ $inversionista->id }}">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger btn-sm" onclick="preguntar{{ $inversionista->id }}(event)">
                ...
                <script>
                    function preguntar{{ $inversionista->id }}(event) {
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                document.getElementById('miFormulario{{ $inversionista->id }}').submit();
                            }
                        });
                    }
                </script>
...
$(function () {
    $("#table1").DataTable({
        ...
        language: {
            ...
            infoEmpty: "Mostrando 0 a 0 de 0 inversionistas",
            infoFiltered: "(filtrado de _MAX_ inversionistas totales)",
